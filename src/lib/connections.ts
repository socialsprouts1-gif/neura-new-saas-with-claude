...
const COLUMNS =
  "id, org_id, phone_number_id, waba_id, display_phone_number, verified_name, label, quality_rating, status, is_default, last_error, last_error_at";
...
async function findRow(
  ...
  const select = `${COLUMNS}, access_token_encrypted`;
  ...
  if (options.connectionId) {
    ...
    if (!data) return { error: "That WhatsApp number is not in this workspace." };
    const row = data as RowWithToken;
  ...
  if (options.phoneNumberId) {
    const { data } = await supabase
      ...
      .maybeSingle();
    if (data) return data as RowWithToken;
  }
  ...
  const row = (fallback ?? [])[0] as RowWithToken | undefined;
  if (!row) {
    return {
