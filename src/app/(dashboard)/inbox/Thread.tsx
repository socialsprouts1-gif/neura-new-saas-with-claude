...
import { toggleConversationBot } from "@/app/(dashboard)/actions";
...
export default function Thread({
  ...
  const toggleBot = () =>
    startTransition(async () => {
      ...
      data.set("id", conversationId);
      data.set("bot_enabled", String(botEnabled));
      await toggleConversationBot(data);
      router.refresh();
    });
            ...
            return (
              <div key={message.id}>
                ...
                <div className={`flex gap-2.5 mb-1.5 ${outbound ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`min-w-0 max-w-[70%] ${outbound ? "items-end" : ""}`}>
                    {showAuthor && (
                      <div
                        className={`text-xs font-semibold mb-1 ${
                          outbound ? "text-right text-accent-ink" : ""
                        }`}
                      >
                        {outbound ? "You" : name || waId}
                      </div>
                    )}
